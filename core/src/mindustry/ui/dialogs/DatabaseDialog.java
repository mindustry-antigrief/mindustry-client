                ...
                for(int j = 0; j < tmpCategory.size; j++){
                    ...
                    sub.table(list -> {
                        ...
                        for(var unlock : array){
                            Image image = unlocked(unlock) || state.isMenu() ? new Image(new TextureRegionDrawable(unlock.uiIcon), mobile ? Color.white : Color.lightGray).setScaling(Scaling.fit) : new Image(Icon.lock, Pal.gray);
