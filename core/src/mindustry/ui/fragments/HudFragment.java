...
public class HudFragment{
    ...
    // List all units cache fields to avoid GC allocations
    private final Seq<Teams.TeamData> presentTeams = new Seq<>();
    private final ObjectMap<UnitType, Integer> unitCounts = new ObjectMap<>();
    private final Seq<UnitType> sortedUnitTypes = new Seq<>();
    private final java.util.Comparator<UnitType> unitTypeComparator = (a, b) -> {
        int tierA = getUnitTier(a);
        int tierB = getUnitTier(b);
        if(tierA != tierB){
            return Integer.compare(tierB, tierA);
        }
        int countA = unitCounts.get(a, 0);
        int countB = unitCounts.get(b, 0);
        if(countA != countB){
            ...
            var listallunitsTable = wavesMain.table(Tex.wavepane, st -> {
                ...
                st.update(() -> {
                    ...
                    for (int i = 0; i < state.teams.present.size; i++) {
                        ...
                        if (team.players.size == 0 && !emptyTeams) continue;
                    ...
                    if (playerTeamData != null) {
                        presentTeams.insert(0, playerTeamData);
                    }

                    // 2. Build team filter row
                    st.table(filters -> {
                        filters.left();
                        for (int i = 0; i < presentTeams.size; i++) {
                            if (i > 0 && i % 10 == 0) {
                                filters.row();
                            }
                            var team = presentTeams.get(i);
                    ...
                    // 3. Build unit lists for each enabled team
                    for (int i = 0; i < presentTeams.size; i++) {
                        var team = presentTeams.get(i);
                        if (disabled.contains(team.team)) continue;
                        
                        // Count unit types to add (single pass)
                        unitCounts.clear();
                        ...
                        st.table(teamRow -> {
                            ...
                            // Sort unit types
                            sortedUnitTypes.clear();
                            unitCounts.keys().toSeq(sortedUnitTypes);
                            sortedUnitTypes.sort(unitTypeComparator);
                            
                            // Limit unit types to 14
                            if (sortedUnitTypes.size > 14) {
                                sortedUnitTypes.truncate(14);
                            }
                            
                            // Add unit icons as cells directly to teamRow
                            for (int j = 0; j < sortedUnitTypes.size; j++) {
                                if (j > 0 && (j - 4) % 5 == 0) {
                                    teamRow.row();
                                }
                                
                                var type = sortedUnitTypes.get(j);
                                int count = unitCounts.get(type, 0);
                                
                                teamRow.table(ut -> {
                                    ut.left().top();
                                    ut.image(type.uiIcon).size(18f).scaling(Scaling.fit).tooltip(type.localizedName);
                                    // Use static text cell instead of dynamic label provider
                                    ut.add(String.valueOf(count)).padLeft(2f).color(Color.white).style(Styles.outlineLabel);
                                }).padRight(6f);
                            }
