...
class ClientLogic {
    ...
    init {
        ...
        Events.on(EventType.PlayerJoin::class.java) { e -> // Run when a player joins the server
            ...
            if (Core.settings.getBool("clientjoinleave") && (Vars.ui.chatfrag.messages.isEmpty || !Strings.stripColors(Vars.ui.chatfrag.messages.first().message).contains("${Regex.escape(Strings.stripColors(e.player.name))}.*connected".toRegex())) && Time.timeSinceMillis(ClientVars.lastJoinTime) > 10000)
        ...
        Events.on(EventType.PlayerLeave::class.java) { e -> // Run when a player leaves the server
            ...
            if (Core.settings.getBool("clientjoinleave") && (Vars.ui.chatfrag.messages.isEmpty || !Strings.stripColors(Vars.ui.chatfrag.messages.first().message).contains("${Regex.escape(Strings.stripColors(e.player.name))}.*disconnected".toRegex())))
