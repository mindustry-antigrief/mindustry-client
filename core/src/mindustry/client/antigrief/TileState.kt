    ...
    fun toElement(): Element {
        ...
        if (configuration != null) {
            ...
            when {
                ...
                config is UnitType -> table.image(config.uiIcon)
                ...
                config is Boolean && block is SwitchBlock -> table.add(if(config) "on" else "off")

                config is Point2 -> table.add("${config.x + x}, ${config.y + y} (${config.x}, ${config.y})")

                config is ByteArray && block is LogicBlock -> table.add(LogicBlock.decompress(config))

                else -> table.label(config.toString().ascii().capLength(20))
