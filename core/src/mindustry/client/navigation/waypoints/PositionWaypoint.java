...
public class PositionWaypoint extends Waypoint implements Position {
    ...
    protected void moveTo(Position target, float circleLength, float smooth){
        ...
        if (Core.settings.getBool("assumeunstrict")) {
            ...
            float length = circleLength <= 0.001f ? 1f : Mathf.clamp((player.unit().dst(target) - circleLength - (player.unit().formation() == null ? 0f : player.unit().formation().pattern.spacing / (float)Math.sin(180f / player.unit().formation.pattern.slots * Mathf.degRad))) / smooth, -1f, 1f);
            vec.setLength((player.unit().formation == null ? player.unit().realSpeed() : player.unit().minFormationSpeed) * length);
