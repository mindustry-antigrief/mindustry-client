...
public class Chain extends Block{
    ...
    public Tile getTileTarget(Tile tile, Item item, Tile src, boolean flip){
        ...
        boolean canForward = to.block().acceptItem(item, to, edge) && to.getTeam() == tile.getTeam() && !(to.block() instanceof Chain);

        if(!canForward || invert){
            Tile a = tile.getNearby(Mathf.mod(from - 1, 4));
            Tile b = tile.getNearby(Mathf.mod(from + 1, 4));
            boolean ac = a != null && a.block().acceptItem(item, a, edge) && !(a.block() instanceof Chain) && a.getTeam() == tile.getTeam();
            boolean bc = b != null && b.block().acceptItem(item, b, edge) && !(b.block() instanceof Chain) && b.getTeam() == tile.getTeam();
